...
$(function() {
    // 静态资源路径
    NY.constant.STATIC_SOURCES_PATH = "/";
});
/*
 * ------------------------
 * 无组件依赖的工具方法
...
NY.dom = (function() {
    ...
    return {
         ...
         * <table id="testTable">
            ...
            <form id="testForm">
                姓名：<input type="text" name="username" value="55" />
                手机：<input type="text" name="mobile" value="66" />
            </form>
...
NY.ajax = function(url, options) {
    ...
    if (options.isCoverSuccess !== true) {
        ...
        settings.success = function(responseData, textStatus, jqXHR) {
            var context = this;
            // 缓存响应数据
            var responseDataText = responseData.info;
            var responseDataTime = 1;
...
NY.biz = {
    ...
    useNoneDataFallback: function(table, nonedataTipTemplate, emptySearchListTipTemplate) {
        ...
        var isEmptySearchList = (nyData.pager && (!NY.string.isEmpty(nyData.pager.searchKey))) ? true : false;
        var tipTemplate = isEmptySearchList ? (emptySearchListTipTemplate || "noneSearchRecordTdTpl") : (nonedataTipTemplate || "nonedataTdTpl");
        ...
        $_contentRowContainer.append(template(tipTemplate, {
            ...
            searchKey: (isEmptySearchList ? nyData.pager.searchKey : ""),
    ...
    initSearchForm: function() {
        $(".ny-search-group").each(function() {
            ...
            if ($(".ny-search-dropdown").length) {
                ...
                var defaultValue = nyData.pager.searchType;
            ...
            // 自动填充 搜索关键字
            if (nyData.filter && !NY.string.isEmpty(nyData.pager.searchKey)) {
                NY.util.getFieldByName("searchKey", $_searchForm).val(nyData.pager.searchKey);
            }
...
NY.component = (function() {
     ...
     *                  在单文件上传模式下，file参数被简化，无需通过数组形式的迭代 调用回调方法后，默认会重新渲染分页页码。你可以在回调方法中显式返回 false 以阻止该默认行为
     *
     * @example 举个栗子
     * <!-- HTML结构 -->
    ...
    var initDropDown = function(options) {
        ...
        $_dropDownContainer.each(function() {
            ...
            var $_nySelectedInput = $(this).next();
            // 下拉菜单选项
